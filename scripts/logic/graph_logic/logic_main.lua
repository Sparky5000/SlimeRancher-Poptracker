...
slime_rancher_location = {}
slime_rancher_location.__index = slime_rancher_location
...
---@type table<string, slime_rancher_new_return>
NAMED_LOCATIONS = {}
local stale = true
...
---@class slime_rancher_new_return
...
---@field exits table<integer, {[1]:slime_rancher_new_return, [2]:fun(): accessibilityLevel}>
---@field keys integer
...
---@param name string
---@return slime_rancher_new_return
function slime_rancher_location.new(name)
	if name == nil then
		error("slime_rancher_location name cannot be nil")
	end
    local self = setmetatable({}, slime_rancher_location)
	self.name = name
    NAMED_LOCATIONS[self.name] = self
...
---@param exit string|slime_rancher_new_return slime_rancher_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function slime_rancher_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = slime_rancher_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
---@param exit string|slime_rancher_new_return slime_rancher_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function slime_rancher_location:connect_two_ways(exit, rule)
...
---@param exit string|slime_rancher_new_return slime_rancher_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function slime_rancher_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
---@param exit string|slime_rancher_new_return slime_rancher_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function slime_rancher_location:connect_two_ways_entrance(name, exit, rule)
...
function slime_rancher_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
---@param rule1? fun(): accessibilityLevel|boolean
---@param rule2? fun(): accessibilityLevel|boolean
function slime_rancher_location:connect_two_ways_stuck(exit, rule1, rule2)
...
---@return accessibilityLevel
function slime_rancher_location:accessibility()
...
---@param keys integer
function slime_rancher_location:discover(accessibility, keys)
...
Entry_Point = slime_rancher_location.new("Entry_Point")
